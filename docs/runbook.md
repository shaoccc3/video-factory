...
3. 建 `.env`（參考 `.env.example`，**不要提交**）：
   ...
   - `PROVIDER_MODE=live`（compose.prod.yaml 預設就是 live）
   ...
   - 選填（留空用預設值）：`DOWNLOAD_ALLOWED_HOSTS`（見第 5 節）、`DOWNLOAD_MAX_BYTES`、`SEEDANCE_TOTAL_TIMEOUT_S`、`PRESIGN_EXPIRES_S`、`ARK_STRIP_AUTH_HEADER`。
     容器只收到 `compose.yaml` 列出的變量，其他寫在 `.env` 的變量不會生效
...
| error_code = download_host_denied | 模型返回的結果地址不在白名單。確認域名後在 `.env` 設 `DOWNLOAD_ALLOWED_HOSTS`（JSON 陣列，例如 `["*.bytepluses.com","*.新域名"]`）；它會整個取代預設清單（見 `backend/app/core/settings.py`），要把仍需要的預設域名一起寫上，再 `up -d` 重建 api、worker。 |
...
| worker 反覆重啟，日誌「worker 臨時目錄不可寫」 | `worker-tmp` 卷的擁有者不是 app（舊版映像建立的卷、或手動改過）。先 `up -d --build` 用新映像；仍失敗時執行一次 `docker compose -f compose.yaml -f compose.prod.yaml run --rm --no-deps --user root worker chown -R app:app /tmp/video-factory-work`，再 `up -d --wait`。 |
